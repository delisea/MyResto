        ...
        stage('Deploy') {
            steps {
                sh '''
					...
					docker run  --hostname ${COMPONENT_NAME}-${COMPONENT_INSTANCE} --name ${COMPONENT_NAME}-${COMPONENT_INSTANCE} --network=25.12.88.85 -p 8080:8080 -d ${COMPONENT_NAME}-${COMPONENT_INSTANCE}:${COMPONENT_VERSION}
				'''
            }
        }
    }
