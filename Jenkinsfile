        ...
        stage('Deploy') {
            steps {
                sh '''
					...
					docker run  --hostname ${COMPONENT_NAME}-${COMPONENT_INSTANCE} --name ${COMPONENT_NAME}-${COMPONENT_INSTANCE} -p 25.12.88.85:8080:8080 -d ${COMPONENT_NAME}-${COMPONENT_INSTANCE}:${COMPONENT_VERSION}
				'''
            }
        }
    }
